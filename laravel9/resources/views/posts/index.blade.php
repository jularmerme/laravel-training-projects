<x-layouts.app title="Blog" meta-description="Blog meta description">
  ...
  <header class="px-6 py-4 space-y-2 text-center">
    ...
    @auth
      <a href="{{ route('posts.create') }}" class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-center text-white uppercase transition duration-500 ease-in-out border border-transparent rounded-md dark:text-sky-200 bg-sky-700 active:bg-sky-900 focus:border-sky-900 focus:shadow-outline-sky">Create new Post</a>
    @endauth
    </header>
    ...
    @foreach ($posts as $post)
      <div class="max-w-3xl px-4 py-2 space-y-4 bg-white rounded shadow dark:bg-slate-800">
        ...
        <div class="flex justify-between">
          <a href="{{ route('posts.edit', $post) }}" class="inline-flex items-center text-xs font-semibold tracking-widest text-center uppercase transition duration-150 ease-in-out dark:text-slate-400 text-slate-500 hover:text-slate-600 dark:hover:text-slate-500 focus:outline-none focus:border-slate-200">Edit</a>
